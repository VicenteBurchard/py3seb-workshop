...
'''
    ...
    resistance_form = resistance_form[0]
    # Create the output variables
    [T_AC, L_n_sub, Ln_C, H, LE, LE_sub, H_sub, LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_x, R_A, delta_Rn, Rn_sub, Rn_C_sub,
      Ln_C_sub, Ln_S, Rn_S, alpha_final, alpha_final_sub, iterations] = [np.zeros(Tr_K.shape) + np.nan for i in range(26)]
    ...
    # radiometric T
    T_C_sub = np.asarray(np.minimum(T_sub, T_A_K), dtype=np.float32)
    flag, T_S = TSEB.calc_T_S(T_sub, T_C_sub, f_theta_sub)
    T_AC = T_A_K.copy()
    ...
    for n_iterations in range(max_iterations):
        ...
        R_A[i], _, _ = TSEB.calc_resistances(
        ...
        while np.any(LE_S[i] < 0):
            ...
            # Calculate aerodynamic resistances
            _, R_x[i], R_S[i] = TSEB.calc_resistances(
                      resistance_form,
                      {"R_A": {"z_T": z_T[i], "u_friction": u_friction[i], "L": L[i],
                               "d_0": d_0[i], "z_0H": z_0H[i]},
                       "R_x": {"u_friction": u_friction[i], "h_C": h_C_sub[i],
            ...
            T_C_sub[i] = TSEB.calc_T_C_series(T_sub[i], T_A_K[i], R_A[i], R_x[i], R_S[i],
            ...
            i = np.logical_and.reduce((LE_S < 0, ~L_converged, flag != F_INVALID))
            # Get air temperature at canopy interface
            T_AC[j] = ((T_A_K[i] / R_A[i] + T_S[i] / R_S[i] + T_C_sub[i] / R_x[i])
                       / (1.0 / R_A[i] + 1.0 / R_S[i] + 1.0 / R_x[i]))

            #calcualte substrate canopy H
            #H_C_sub[j] = rho[j] * c_p[j] * (T_C_sub[j] - T_AC[j]) / R_x[j]
            # Calculate soil fluxes
            H_S[j] = rho[i] * c_p[i] * (T_S[i] - T_AC[i]) / R_S[i]

            # Compute Soil Heat Flux Ratio
            G[i] = TSEB.calc_G([calcG_params[0], calcG_array], Rn_S, j)
            ...
            LE[j] = np.asarray(LE_C[i] +LE_C_sub[i]+ LE_S[i])
    ...
    return (flag, T_S, T_C_sub, T_AC, L_n_sub, Ln_C, Ln_C_sub, Ln_S,  LE_C, H_C,
            LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_x, R_A, u_friction, L, n_iterations)
...
'''
    ...
    resistance_form = resistance_form[0]
    # Create the output variables
    [flag,T_AC, L_n_sub, L_nC, H, LE, LE_sub, H_sub, LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_x, R_A,
     Rn_sub, Rn_C_sub, Ln_C_sub, Ln_S, Rn_S, iterations] = [np.zeros(T_C.shape) + np.nan for i in range(24)]
    ...
    Ln_C_sub, _ = rad.calc_L_n_Campbell(T_C_sub, T_S, L_dn, LAI, emis_sub, emis_S, x_LAD=x_LAD)
    ...
    return flag, T_AC, L_n_sub, Ln_C, Ln_C_sub, Ln_S,  LE_C, H_C, LE_C_sub, H_C_sub, LE_S, H_S, G, R_S, R_x, R_A, u_friction,L, n_iterations
